...
Begin VB.Form frmMap 
   ...
   Begin VB.CommandButton cmdLoadBuffer 
      Caption         =   "Reload &Buffer"
      Height          =   375
      Left            =   5160
      TabIndex        =   27
      Top             =   4920
...
'
' What's new in EasyMapDraw 2.0 example ?
' ---------------------------------------
...
' * buffer added for caching coördinates
'
Option Explicit

' buffer
' ------
Private Type typCoordinate
  BreakLine As Boolean
  X As Double
  Y As Double
  Color As ColorConstants
End Type
Private BufferTel As Long 'UBound gives error, so we use BufferTel
Private Buffer() As typCoordinate

Private Sub addElementToBuffer(vBreakLine As Boolean, _
Optional vLngDeg As Double, Optional vLatDeg As Double, _
Optional vColor As ColorConstants)
  BufferTel = BufferTel + 1
  ReDim Preserve Buffer(1 To BufferTel) As typCoordinate
  If vBreakLine = True Then
    Buffer(BufferTel).BreakLine = True
  Else
    Buffer(BufferTel).Color = vColor
    Buffer(BufferTel).X = vLngDeg
    Buffer(BufferTel).Y = vLatDeg
  End If
End Sub
...
Private Sub EasyMapDraw_ZoomConfigured(vZoomdirection As enmZoomDirection)
  ...
  End Select
  '
  mdlInterface.LockControls Me, True
  EasyMapDraw.DrawEquatorMeridian DrawEquatorAndMeridian
  '
  If BufferTel > 0 Then
    Do
      tel = tel + 1
      If Buffer(tel).BreakLine = True Then
        EasyMapDraw.BreakLine
      Else
        EasyMapDraw.DrawColor = Buffer(tel).Color
        EasyMapDraw.DrawPoint Buffer(tel).X, Buffer(tel).Y
      End If
      '
      pgbProgress.Value = CByte(tel / BufferTel * 100)
      '
    Loop While tel < BufferTel And EasyMapDraw.IsDrawing
  End If
  '
  mdlInterface.LockControls Me, False
...
Private Sub Form_Click()
  EasyMapDraw.DrawLine -100, 80, 100, -80
  EasyMapDraw.EndDraw
End Sub
...
Private Sub cmdLoadBuffer_Click()
  ...
  If tmpFile <> "" Then
    ...
    Do
      ...
      '
      ' interrupt drawing of a line between two points
      addElementToBuffer True
      '
      ...
      If Not EOF(F) Then
        Do
          ...
          If A = vbNullString Then addElementToBuffer True
          '
          I = InStr(1, A, " ")
          If I > 0 Then
             ...
             If latDeg <> 0 Then
                ...
                '
                addElementToBuffer False, lngDeg, latDeg, Color
                '
             End If
          End If
      ...
      'DoEvents
    Loop While tmpFile <> ""
    strStatus.Panels(1).Text = "Loading card-fragments completed"
    mdlInterface.LockControls Me, False
  Else
